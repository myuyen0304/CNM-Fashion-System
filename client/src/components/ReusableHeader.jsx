...
export default function ReusableHeader({
  ...
  const userInitial = String(userMenu?.displayName || "U")
    .trim()
    .charAt(0)
    .toUpperCase();
  ...
  const userMenuControl = userMenu ? (
          ...
          {userMenu.items?.map((item) =>
            item.to ? (
              ...
              <button
                ...
                className="block w-full rounded px-3 py-2 text-left text-sm normal-case text-gray-700 hover:bg-gray-100 hover:text-primary"
              >
                {item.label}
              </button>
            ),
